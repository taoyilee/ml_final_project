...
if __name__ == "__main__":
    ...
    for r in range(runs):
        for act in activation:
            ...
            model.add(
                Dense(units=nodes_per_hidden, activation=act, kernel_regularizer=regularizers.l2(regularization),
                      input_dim=x_trn_raw.shape[1]))
            for h in range(hidden_layers):
                model.add(Dense(units=nodes_per_hidden, activation=act,
            ...
            history.history["hidden_layers"] = hidden_layers
            history.history["nodes_per_hidden"] = nodes_per_hidden
            history.history["train_percent"] = trn_p
            history.history["dev_percent"] = dev_p
            history.history["act"] = act
            ...
            np.savetxt(f"{exp_dir}/Y_submit.txt", Yte, ' %d, %.4f ', header=' ID,Prob1 ', comments='',
                       delimiter=',')
            df.to_csv("problem_5b_summary.csv")
