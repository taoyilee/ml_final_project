...
if __name__ == "__main__":
    ...
    if not os.path.isfile(csv_out):
        ...
        df = pd.read_csv(csv_out, usecols=["trn_p", "dev_p", "k", "trn_auc", "dev_auc", "alpha", "scaled"])
    k = list(range(1, 71, 2))
    print(k)
    ...
    for i in range(resamping):
        training_auc[i], validating_auc[i] = sample_train_knn(k, raw_data, trn_p, dev_p, rescale=False,
                                                              alpha=alpha)
        data_table = np.vstack((trn_p * np.ones_like(training_auc[i]), dev_p * np.ones_like(training_auc[i]),
                                ...
                                False * np.ones_like(training_auc[i]))).T
        df = df.append(
    ...
    with open(f"learners/training_auc_3b.pickle", "wb") as f:
        pickle.dump(training_auc, f)
    with open(f"learners/validating_auc_3b.pickle", "wb") as f:
